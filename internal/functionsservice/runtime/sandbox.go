	...
	if ctx.Services != nil {
		...
		secretsObj["get"] = func(secretName string) interface{} {
			if ctx.Services.Secrets != nil {
				...
				value, err := secretservice.GetSecret(context.Background(), ctx.FunctionID, secretName)
				if err != nil {
				...
				// The Mocksecretservice.GetSecret (via Get) returns "" if not found.
				if value == "" {
		...
		secretsObj["set"] = func(secretName string, secretValue string) interface{} {
			if ctx.Services.Secrets != nil {
				...
				// Type assert
				secretservice, ok := ctx.Services.Secrets.(SecretsSetter)
				if !ok {
				...
				err := secretservice.SetSecret(context.Background(), ctx.FunctionID, secretName, secretValue)
				if err != nil {
		...
		secretsObj["delete"] = func(secretName string) interface{} {
			if ctx.Services.Secrets != nil {
				...
				// Type assert
				secretservice, ok := ctx.Services.Secrets.(SecretsDeleter)
				if !ok {
				...
				err := secretservice.DeleteSecret(context.Background(), ctx.FunctionID, secretName)
				if err != nil {
