...
import (
	...
	transaction "github.com/r3e-network/neo_service_layer/internal/transactionservice"
...
// Mocksecretservice implements a basic secrets service for testing
...
func (m *Mocksecretservice) Get(secretName string) (string, error) {
...
func (m *Mocksecretservice) Set(secretName, value string) error {
...
func (m *Mocksecretservice) Delete(secretName string) error {
	m.mu.Lock() // Write lock
	defer m.mu.Unlock()
...
func (m *Mocksecretservice) GetSecret(ctx context.Context, functionID, secretName string) (string, error) {
	// m.Get handles its own locking
	return m.Get(secretName)
}

// SetSecret is the method expected by the sandbox
func (m *Mocksecretservice) SetSecret(ctx context.Context, functionID, secretName, value string) error {
	// m.Set handles its own locking
	return m.Set(secretName, value)
}

// DeleteSecret is the method expected by the sandbox
func (m *Mocksecretservice) DeleteSecret(ctx context.Context, functionID, secretName string) error {
...
func TestServiceInteroperabilityWithRealSandbox(t *testing.T) {
	...
	err := sandboxTest.mocksecretservice.Set("test-api-key", "secret-api-key-12345")
	...
	t.Run("secretservice", func(t *testing.T) {
		code := `
			function main() {
		...
		// _, err = sandboxTest.mocksecretservice.Get("new-secret")
		...
		valAfterDelete, err := sandboxTest.mocksecretservice.Get("new-secret")
...
func TestCombinedServiceInteroperabilityWithRealSandbox(t *testing.T) {
	...
	err := sandboxTest.mocksecretservice.Set("api-key", "test-api-key-value")
	require.NoError(t, err)
	err = sandboxTest.mocksecretservice.Set("wallet-password", "test-wallet-password")
	require.NoError(t, err)
	...
	lastSwapTx, err := sandboxTest.mocksecretservice.Get("last-swap-tx")
	assert.NoError(t, err)
